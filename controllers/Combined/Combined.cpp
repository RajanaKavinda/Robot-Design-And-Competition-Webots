...
float line_follow_PID[3]={10,0,0};  //kp,ki,kd

//wall following arrays
float wall_follow_error_array[2]={0,0};   //previous_error , Integral
float wall_follow_PID[3]={0.01,0.0,0.0};   //kp,ki,kd

//robot functions
void line_following();
void wall_following();
void turnMotors(double);
...
{  
   ...
   while ((robot->step(TIME_STEP) != -1) && isLinefollowing==true) 
   {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        if (t_count==8) {isLinefollowing=false;}
        
        
        cout <<isLinefollowing<<endl;
...
{ 
  ...
  while (robot->step(TIME_STEP) != -1 && isLinefollowing==false ) {

    // Read distance sensor values
    
    if ((dis[1]->getValue() > dis[2]->getValue()) && dis[3]->getValue() > 300.0) 
    {
      dis_readings[0] = 1;
      dis_readings[1] = 1;
      dis_readings[2] = 0;
    ...
    else if ( (dis[2]->getValue() > dis[1]->getValue()) && dis[0]->getValue()> 300.0)
    {
      dis_readings[0] = 0;
      dis_readings[1] = 0;
      dis_readings[2] = 1;
